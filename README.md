...
- Data-Oriented Architecture & Cache Locality: Achieved 100k boids using rayon for data-parallel multithreading over flat arrays. Implemented implicit AABBs—calculating bounds dynamically via integer math (Vector3i) from a RootAABB rather than storing them per-node. Combined with a coarse/fine grid where only leaf nodes handle positions, this drastically reduced memory footprint and maximized CPU cache locality.
