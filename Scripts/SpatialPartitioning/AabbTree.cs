...
example:   `Debug.Assert()` replace with `Debug.Assert()`
...
{
    public static bool _TryRemove<TKey, TValue>(this Dictionary<TKey, TValue> dict, TKey key, out TValue value)
    {
...
public static class AabbExtensions
{
    ...
    public static Aabb _Union(this Aabb aabb, Aabb other)
    {
        // Component-wise min for position
        float minX = Mathf.Min(aabb.Position.X, other.Position.X);
        float minY = Mathf.Min(aabb.Position.Y, other.Position.Y);
        float minZ = Mathf.Min(aabb.Position.Z, other.Position.Z);
        Vector3 newPosition = new Vector3(minX, minY, minZ);

        // Component-wise max for end points
        float maxX = Mathf.Max(aabb.End.X, other.End.X);
        float maxY = Mathf.Max(aabb.End.Y, other.End.Y);
        float maxZ = Mathf.Max(aabb.End.Z, other.End.Z);
        Vector3 newEnd = new Vector3(maxX, maxY, maxZ);

        return new Aabb(newPosition, newEnd - newPosition);
    }

    public static Aabb _Scaled(this Aabb aabb, float scale)
    {
        Vector3 center = aabb.Position + (aabb.Size / 2);
        Vector3 scaledSize = aabb.Size * scale;
        return new Aabb(center - (scaledSize / 2), scaledSize);
    }
    /// <summary>
...
public partial class AABBTree<TData> where TData : class
{
    ...
    public int Count
    {
        ...
        {
            this._AssertTreeOk();
            //Debug.Assert(nodeCount == nodeIndexLookup.Count);
    ...
    public bool MoveNode(TData data, Aabb aabb, bool autoDisplacement = false, bool forceMove = false)
    {
        ...
        {
            GD.PrintErr("item not found in AabbTree");
        }
    ...
    private bool MoveNodeInternal(int node, Aabb itemAabb, Vector3 displacement, bool forceMove = false)
    {
        ...
        GD.PrintErr(node >= 0 && node < nodeCapacity);
        GD.PrintErr(nodes[node].IsLeaf);
    ...
    private void RemoveLeaf(int leaf)
    {
        ...
        Debug.Assert(leaf >= 0 && leaf < nodeCapacity); // Ensure the leaf index is valid.
        Debug.Assert(nodes[leaf].IsLeaf); // Ensure the node being removed is actually a leaf.
        ...
        {
            Debug.Assert(root == leaf); // Assert that the root is indeed the leaf.
        ...
        else
        {
            ...
            GD.PrintErr("Removed leaf is not a child of its parent!");
        }
    ...
    {
        GD.PrintErr(nodeA >= 0 && nodeA < nodeCapacity);
        GD.PrintErr(nodeB >= 0 && nodeB < nodeCapacity);
...
public partial class AABBTree<TData> where TData : class
{
    ...
    private void _AssertAlive(int index, bool doNotIgnoreNullNode = false)
    {
        if (index == NULL_NODE)
        {
            ...
            {
                Debug.Assert(false, "index is NullNode");
            }
        }

        var nodeObj = nodes[index];
        Debug.Assert(nodeObj.Parent != index);
    }
    ...
    public void _AssertTreeOk(bool ignoreAabbConsistency = false)
    {
        ...
        for (int i = 0; i < nodeCapacity; i++)
        {
            ...
            if (nodes[i].IsLeaf)
            {
                ...
                Debug.Assert(nodes[i].Child1 == NULL_NODE);
                Debug.Assert(nodes[i].Child2 == NULL_NODE);
            }
            else
            {
                ...
                Debug.Assert(nodes[nodes[i].Child1].Parent == i);
                Debug.Assert(nodes[nodes[i].Child2].Parent == i);
            }
            ...
            if (i == root)
            {
                ...
                Debug.Assert(nodes[i].Parent == NULL_NODE);
            }
            else
            {
                Debug.Assert(nodes[i].Parent >= 0 && nodes[i].Parent < nodeCapacity);
            }
        }
        ...
        Debug.Assert(actualNodeCount == nodeCount);
    ...
    private void _AssertNoCycles(int nodeIndex)
    {
        ...
        while (nodes[current].Parent != NULL_NODE)
        {
            ...
            {
                Debug.Assert(false, "Cycle detected in the tree starting from node: " + nodeIndex);
    ...
    private void _AssertAabbConsistent(int nodeIndex)
    {
        ...
        if (nodes[nodeIndex].IsLeaf)
        {
            ...
            Debug.Assert(nodes[nodeIndex].Data is not null);
            var aabbBufferedSize = nodes[nodeIndex].Aabb._Scaled(1.01f); //in case of rounding errors?
            Debug.Assert(aabbBufferedSize.Encloses(nodes[nodeIndex].ItemAabb));
        }
        else
        {
            ...
            Debug.Assert(aabbBufferedSize.Encloses(child1Aabb));
            Debug.Assert(aabbBufferedSize.Encloses(child2Aabb));
